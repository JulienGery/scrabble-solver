...
auto getLettersCount(const std::string &letters,
...
void readFile(const std::string &path, std::vector<t> &buffer) {
  ...
  if (!file.is_open())
    throw std::runtime_error("\033[1;31mFailed to open file " + path +
                             "\033[0m");
...
void readFile(const std::string &path, std::vector<std::string> &buffer) {
  ...
  std::copy(std::istream_iterator<std::string>(file),
            std::istream_iterator<std::string>(), std::back_inserter(buffer));
  file.close();
}
...
int main(int argc, char *argv[]) {
  if (argc != 2)
    throw std::runtime_error("\033[1;31mGive letters!\033[0m");

  std::vector<uint32_t> WBRp;
  readFile("WBRp.bin", WBRp);
  std::vector<uint16_t> lengths;
  readFile("lengths.bin", lengths);
  std::vector<std::string> words;
  readFile("wordsSorted.txt", words);
  std::vector<uint16_t[26]> lettersCount(
      words.size()); // size must be set at init time
  ...
  size_t eligible = 0;
  std::vector<size_t> indexes(lengths.size());
  ...
  case 0:
    for (size_t i = 0; i < lengths.size(); i++) {
      if (!(~(~WBRp[i] | representation)) && // faster to put !(~(~WBRp[i] | representation)) first.
          (length >= lengths[i])) { 
        indexes[eligible] = i;
        eligible++;
      }
  ...
  std::cout
      << "\033[36m" << delta.count()
      << " nanoseconds \033[1;36mwithout accounting for files reading\n\033[0m";
